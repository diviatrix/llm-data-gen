...
class AuthManager {
  ...
  // Helper methods for synchronous better-sqlite3 API
  _run(sql, params = []) {
  ...
  async createTables() {
    ...
    const columnInfo = this._all('PRAGMA table_info(users)');
    ...
    if (!hasRoleId) {
      ...
      this._run('ALTER TABLE users ADD COLUMN role_id INTEGER DEFAULT 2');
    ...
    if (!hasStorageUsed) {
      ...
      this._run('ALTER TABLE users ADD COLUMN storage_used INTEGER DEFAULT 0');
    }

    if (!hasStorageQuota) {
      // Add storage_quota column to existing users table (10MB default)
      this._run('ALTER TABLE users ADD COLUMN storage_quota INTEGER DEFAULT 10485760');
    }

    if (!hasSettings) {
      // Add settings column to existing users table
      this._run('ALTER TABLE users ADD COLUMN settings TEXT DEFAULT \'{}\'');
    }

    if (!hasPasswordHash) {
      // Add password_hash column to existing users table
      this._run('ALTER TABLE users ADD COLUMN password_hash TEXT');

      // If there are existing users with old password column, migrate them
      if (hasPassword) {
        const existingUsers = this._all('SELECT id, password FROM users WHERE password IS NOT NULL AND password_hash IS NULL');
        for (const user of existingUsers) {
          // Hash the existing plaintext password
          const hashedPassword = await bcrypt.hash(user.password, 10);
          this._run('UPDATE users SET password_hash = ? WHERE id = ?', [hashedPassword, user.id]);
        }
        ...
        const existingUsers = this._all('SELECT id FROM users WHERE password_hash IS NULL');
    ...
    // Create indexes
    this._run('CREATE INDEX IF NOT EXISTS idx_api_keys_user_id ON api_keys(user_id)');
    this._run('CREATE INDEX IF NOT EXISTS idx_sessions_user_id ON sessions(user_id)');
    this._run('CREATE INDEX IF NOT EXISTS idx_sessions_expires_at ON sessions(expires_at)');
  }
  ...
  async createUser(email, password) {
    ...
    try {
      ...
      const result = this._run(query, params);
  ...
  getRoles() {
    return this._all('SELECT * FROM roles ORDER BY id');
  }
  ...
  getAllUsers() {
    return this._all(
      `SELECT u.id, u.email, u.created_at, u.last_login, u.is_active, u.storage_used, u.storage_quota,
              u.role_id, r.name as role_name, u.settings,
  ...
  getStorageInfo(userId) {
    const user = this._get(
      'SELECT storage_used, storage_quota, settings FROM users WHERE id = ?',
      [userId]
    );
  ...
  canUploadFile(userId, fileSize) {
    const storage = this.getStorageInfo(userId);
    return storage.available >= fileSize;
  }

  addStorageUsed(userId, bytes) {
    this._run(
  ...
  subtractStorageUsed(userId, bytes) {
    this._run(
  ...
  deleteActiveGeneration(id) {
    this._run('DELETE FROM active_generations WHERE id = ?', [id]);
  }

  getActiveGenerations(userId = null) {
    const query = userId
    ...
    const rows = this._all(query, userId ? [userId] : []);
  ...
  saveGenerationHistory(userId, data) {
    const result = this._run(
  ...
  deleteGenerationHistory(userId, generationId) {
    // Verify the generation belongs to the user before deleting
    const generation = this._get(
  ...
  async saveApiKey(userId, apiKey) {
    ...
    const user = this._get('SELECT password_hash FROM users WHERE id = ?', [userId]);
    ...
    this._run('DELETE FROM api_keys WHERE user_id = ?', [userId]);

...
export class CloudUserPaths {
  ...
  static getUserConfigsDir(userId) {
    return UserStorage.getUserFilesDir(userId);
  }
  ...
  static async getUserStorageInfo(userId) {
    ...
    return {
      ...
      outputSize: await getDirectorySize(UserStorage.getUserFilesDir(userId)),
      configSize: await getDirectorySize(UserStorage.getUserFilesDir(userId))
    };
  }
  ...
  static getPathsForRequest(req) {
    ...
    if (req) {
      ...
      return {
        ...
        getUserConfigsDir: () => UserStorage.getUserFilesDir(userId),
        getUserOutputDir: () => UserStorage.getUserFilesDir(userId),
        resolveOutputPath: (path) => this.resolveOutputPath(path, userId),
        findConfigFile: async (filename) => {
          const userConfigPath = path.join(UserStorage.getUserFilesDir(userId), filename);
          if (await UserStorage.fileExists(userConfigPath)) {
            return userConfigPath;
          }

          const userExamplePath = path.join(UserStorage.getUserFilesDir(userId), 'examples', filename);

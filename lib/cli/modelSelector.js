...
async function showFilteredModels(models, filterType, currentModel) {
  ...
  if (model === 'other') {
    ...
    await UserStorage.savePreferences(0, { lastModel: customModel });
    return customModel;
  }
  
  await UserStorage.savePreferences(0, { lastModel: model });
  return model;
}
...
async function showAllModels(models, currentModel) {
  ...
  await UserStorage.savePreferences(0, { lastModel: allModel });
  return allModel;
}

export async function selectModel(currentModel) {
  // If no current model, try last chosen or default to auto
  if (!currentModel || currentModel === 'openrouter/auto') {
    const prefs = await UserStorage.getPreferences(0);
    if (prefs.lastModel && prefs.lastModel !== 'openrouter/auto') {
      currentModel = prefs.lastModel;
    } else {

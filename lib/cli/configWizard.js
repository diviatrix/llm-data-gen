...
export async function selectConfig(configManager) {
  ...
  // Start at user's configs directory
  const userConfigsPath = UserPaths.getUserConfigsDir();
  ...
  while (!selectedConfig) {
    ...
    try {
      ...
      // Show current path (relative to user home)
      const displayPath = path.relative(UserPaths.getUserDataDir(), currentPath) || 'llmdatagen';

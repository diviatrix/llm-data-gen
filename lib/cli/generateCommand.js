...
export async function executeGenerate(configManager, options) {
  try {
    ...
    // Fast-fail for non-interactive runs when there is no API key available.
    // Avoid attempting multiple generation items when the API key is missing
    // (default tasks often include multiple items, which would produce repeated
    // errors). Tests and CI expect a quick error in this case.
    const hasApiKey = (config.api && config.api.apiKey) || process.env.OPENROUTER_API_KEY;
    if (options.interactive === false && !hasApiKey) {
      throw new AppError('OpenRouter API key is required for non-interactive generation. Set OPENROUTER_API_KEY or provide apiKey in the config.', 'API_KEY_MISSING');
    }

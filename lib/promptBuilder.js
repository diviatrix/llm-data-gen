export class PromptBuilder {
  ...
  buildStructurePrompt(schema, context = {}) {
    const structure = this._schemaToStructure(schema, context);
  ...
  _schemaToStructure(schema, context = {}) {
    if (schema.type === 'object' && schema.properties) {
      const obj = {};
      for (const [key, prop] of Object.entries(schema.properties)) {
        if (prop['x-llm-generate']) {
          obj[key] = this._getSchemaFieldDescription(prop['x-llm-generate'], this._getTypeDescription(prop), context);
        } else {
          obj[key] = this._schemaToStructure(prop, context);
        }
      }
      return obj;
    }
    ...
    if (schema.type === 'array' && schema.items) {
      return [this._schemaToStructure(schema.items, context)];
    }

    if (schema['x-llm-generate']) {
      return this._getSchemaFieldDescription(schema['x-llm-generate'], this._getTypeDescription(schema), context);
    }

    return `<${this._getTypeDescription(schema)}>`;
  }

  _getTypeDescription(schema) {
    // Handle union types (anyOf)
    if (schema.anyOf && Array.isArray(schema.anyOf)) {
      const types = schema.anyOf.map(item => item.type).filter(Boolean);
      return types.join(' | ');
    }

    // Handle oneOf types
    if (schema.oneOf && Array.isArray(schema.oneOf)) {
      const types = schema.oneOf.map(item => item.type).filter(Boolean);
      return types.join(' | ');
    }

    // Handle allOf types (less common but possible)
    if (schema.allOf && Array.isArray(schema.allOf)) {
      const types = schema.allOf.map(item => item.type).filter(Boolean);
      return types.join(' & ');
    }
  ...
  _getSchemaFieldDescription(generateConfig, fieldType, context) {
    if (generateConfig.template) {
      return this._resolveTemplate(generateConfig.template, context);
    }

    if (generateConfig.prompt) {
      return this._resolveTemplate(generateConfig.prompt, context);
    }

    if (generateConfig.description) {
      return this._resolveTemplate(generateConfig.description, context);
    }

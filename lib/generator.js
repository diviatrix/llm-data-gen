...
export class DataGenerator {
  ...
  async _saveResults(key, results, task) {
    ...
    const outputPath = UserPaths.resolveOutputPath(this.config.output.outputPath);
  ...
  _printSummary(duration) {
    ...
    if (this.totalTokens > 0) {
      ...
      console.log(`${chalk.gray('📊')} Average tokens/item: ${chalk.bold(avgTokens.toLocaleString())}`);
    }
    ...
    if (this.totalCost > 0) {
      ...
      console.log(`${chalk.gray('💵')} Average cost/item: ${chalk.bold('$' + avgCost.toFixed(4))}`);
    }
    ...
    if (this.errors.length > 0) {
      ...
      if (formattedErrors.length > 10) {
        console.log(`  ... and ${formattedErrors.length - 10} more`);
      }
    }

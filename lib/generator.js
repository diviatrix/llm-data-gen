...
import { DiagnosticsTracker, PerformanceTracker } from './diagnostics.js';
import { generateDataStream, createProcessingPipeline } from './streaming/dataStream.js';
...
export class DataGenerator {
  constructor(config, context = {}) {
    ...
    this.abortSignal = context.abortSignal || null;
    this.useStreaming = context.useStreaming || false;
    ...
    if (this.apiConfig?.model) {
      console.info(`[DataGenerator] Using model: ${this.apiConfig.model}`);
    }
  ...
  async generate(task) {
    const rawLabel = task.theme || task.topic || task.category || 'items';
    // Build context first to get all available variables
    const context = this._buildContext(task, 0);
    // Process the label with template variables, but exclude the main field to avoid recursion
    const processedLabel = this._processTaskLabel(rawLabel, context, task);

    const spinner = ora(`Generating ${task.count} items for ${processedLabel}`).start();
    const results = [];
    ...
    try {
      for (let i = 0; i < task.count; i++) {
        ...
        const currentContext = this._buildContext(task, i);
        const currentLabel = this._processTaskLabel(rawLabel, currentContext, task);
        spinner.text = `Generating item ${i + 1}/${task.count} for ${currentLabel}`;

        const item = await this._generateSingleItem(currentContext);
      ...
      spinner.succeed(`Generated ${results.length} items for ${processedLabel}`);
      return results;
    } catch (error) {
      spinner.fail(`Failed to generate items for ${processedLabel}: ${error.message}`);
  ...
  async generateAll() {
    ...
    // Save to generation history if in web context
    if (this.context.req?.user?.userId) {
      try {
        const { authManager } = await import('./auth.js');
        const auth = await authManager;

        // Collect output files with relative paths
        const outputFiles = [];
        const userId = this.context.req?.user?.userId || 0;

        for (const key of Object.keys(allResults)) {
          const task = this.config.generation.tasks.find(t => this._getTaskKey(t) === key);
          if (task && this.config.output?.outputPath) {
            // Get the full output path like in _saveResults
            let outputPath;
            if (this.config.output?.outputPath) {
              outputPath = path.isAbsolute(this.config.output.outputPath)
                ? this.config.output.outputPath
                : path.join(UserStorage.getUserFilesDir(userId), this.config.output.outputPath);
            } else {
              outputPath = UserStorage.getUserFilesDir(userId);
            }
            ...
            const userOutputDir = UserStorage.getUserFilesDir(userId);
            const fullFilePath = path.join(outputPath, fileName);
            const relativePath = path.relative(userOutputDir, fullFilePath);

            outputFiles.push(relativePath);
          }
        }

        await auth.saveGenerationHistory(this.context.req.user.userId, {
          configName: this.config.meta?.name || this.config.meta?.description || 'Unnamed config',
          model: this.apiConfig?.model || 'unknown',
          status: this.errors.length > 0 ? 'completed_with_errors' : 'completed',
          itemsGenerated: this.generatedCount,
          ...
          metadata: {
            duration,
            format: this.format,
            tasks: this.config.generation.tasks.length
          }
        });
      } catch (error) {
  ...
  async _generateSingleItem(context) {
    const tracker = new PerformanceTracker('generation');

    try {
      DiagnosticsTracker.trackGeneration('start', this.config, { context });
      tracker.start('total');
      ...
      const result = await this.apiClient.generateJSON(systemPrompt, userPrompt, {
        verbose: true, // Always get detailed info
        format: this.format
      });
      ...
      if (result.data) {
        ...
        if (usage) {
          ...
          // Get actual model used (might differ from requested)
          const actualModel = result.model || this.config.api?.model;

          // Calculate cost based on actual pricing from response if available
          let cost = 0;
          if (usage.prompt_tokens_cost && usage.completion_tokens_cost) {
            // Use actual cost from API if provided
            cost = usage.prompt_tokens_cost + usage.completion_tokens_cost;
          } else {
            // Fallback to manual calculation
            const modelPrice = await this._getModelPrice(actualModel);
            if (modelPrice > 0) {
              cost = (totalTokens / 1000000) * modelPrice;
            }
          }
          ...
          // Log detailed info for each request
          console.info('  📊 Request completed:');
          console.debug(`     Model: ${actualModel}${actualModel.includes(':online') ? ' (with web search)' : ''}`);
          console.debug(`     Tokens: ${promptTokens} prompt + ${completionTokens} completion = ${totalTokens} total`);
          console.debug(`     Cost: ${console.cost(cost)}${usage.web_search_units ? ` + ${usage.web_search_units} web searches` : ''}`);
      ...
      DiagnosticsTracker.trackValidation('start', this.config.schema);
      const validation = this.validator.validate(processedResult);
      DiagnosticsTracker.trackValidation('complete', this.config.schema, { valid: validation.valid });

      if (!validation.valid) {
        throw new Error(`Validation failed: ${validation.errors.join(', ')}`);
      }

      const duration = tracker.end('total');
      DiagnosticsTracker.trackGeneration('complete', this.config, { duration, success: true });

      return processedResult;
    } catch (error) {
      const duration = tracker.end('total');
      DiagnosticsTracker.trackGeneration('error', this.config, { duration, error: error.message });
  ...
  async _saveResults(key, results, task) {
    ...
    // Get userId from context (web) or use 0 for CLI
    const userId = this.context.req?.user?.userId || 0;
  ...
  _printSummary(duration) {
    ...
    // Model info
    if (this.apiConfig?.model) {
      console.info(`Model used: ${this.apiConfig.model}`, '🤖');
    }
    ...
    if (this.totalCost > 0) {
      ...
      if (this.context.estimatedCost) {
        const estimatedTotal = parseFloat(this.context.estimatedCost);
        const difference = this.totalCost - estimatedTotal;
        const percentDiff = ((difference / estimatedTotal) * 100).toFixed(1);
        console.debug(`Estimated vs Actual: ${console.cost(estimatedTotal)} vs ${console.cost(this.totalCost)} (${percentDiff > 0 ? '+' : ''}${percentDiff}%)`);
      }
    } else {
      console.info('Total cost: Free', '💰');
    }
  ...
  _processTaskLabel(label, context, task) {
    // Don't process the main task field to avoid recursion
    // But process all other template variables like {date}, {datetime}, etc.
    const contextForLabel = { ...context };

    // Remove the main task field that contains the label to avoid recursion
    if (task.theme && label === task.theme) {
      delete contextForLabel.theme;
      delete contextForLabel.theme_translit;
    } else if (task.topic && label === task.topic) {
      delete contextForLabel.topic;
      delete contextForLabel.topic_translit;
    } else if (task.category && label === task.category) {
      delete contextForLabel.category;
      delete contextForLabel.category_translit;
    }

    // Process the label with remaining context variables
    return this.promptBuilder._resolveTemplate(label, contextForLabel);
  }
  ...
  async generateWithStreaming() {
    ...
    for (const task of this.config.generation.tasks) {
      const contexts = this._generateContexts(task);
      const key = task.key || Object.keys(allResults).length.toString();

      const spinner = ora(`Streaming ${task.count} items for ${task.theme || task.topic || key}...`).start();

      try {
        const { results, stats } = await createProcessingPipeline(
          this.config,
          this.apiClient,
          contexts,
          {
            batchSize: this.config.api?.batchSize || 3,
            delay: this.config.api?.batchDelay || 1000,
            onProgress: (count, context) => {
              spinner.text = `Generated ${count}/${task.count} items`;
              if (this.onProgress) {
                this.onProgress({
                  current: count,
                  total: task.count,
                  currentItem: context.theme || context.topic || 'item'
                });
              }
            }
          }
        ...
        this.generatedCount += stats.generatedCount;
        this.errors.push(...stats.errors);

        allResults[key] = results;
        await this._saveResults(key, results, task);

        spinner.succeed(`Streamed ${results.length} items (${stats.errorCount} errors)`);
      } catch (error) {

...
export class ConfigManager {
  ...
  async loadDefaultConfig() {
    try {
      const userDefaultPath = path.join(UserStorage.getUserFilesDir(0), 'default.json');
  ...
  async listExamples() {
    try {
      const userExamplesPath = path.join(UserStorage.getUserFilesDir(0), 'examples');
  ...
  async loadExample(name) {
    ...
    try {
      const configPath = path.join(UserStorage.getUserFilesDir(0), 'examples', filename);
  ...
  _getMinimalConfig() {
    return {
      ...
      output: {
        type: 'array',
        outputPath: UserStorage.getUserFilesDir(0)
      },
      schema: {
        type: 'object',
        properties: {}

...
export class ConfigManager {
  ...
  async loadDefaultConfig() {
    try {
      // Try user default config first
      const userDefaultPath = await UserPaths.findConfigFile('default.json');
  ...
  async listExamples() {
    try {
      // Try user examples first
      const userExamplesPath = path.join(UserPaths.getUserConfigsDir(), 'examples');
      try {
        const userFiles = await fs.readdir(userExamplesPath);
        const userExamples = userFiles.filter(f => f.endsWith('.json'));
        if (userExamples.length > 0) {
          return userExamples;
        }
      } catch {
        // User examples directory doesn't exist
      }
      
      // Fallback to system examples
  ...
  async loadExample(name) {
    ...
    try {
      // Use findConfigFile to check user and system paths
      const configPath = await UserPaths.findConfigFile(path.join('examples', filename));
  ...
  _getMinimalConfig() {
    return {
      ...
      output: {
        type: 'array',
        outputPath: UserPaths.getUserOutputDir()
      },
      schema: {
        type: 'object',
        properties: {}

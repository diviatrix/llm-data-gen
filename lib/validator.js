...
import { getValidatorPool } from './workers/validatorPool.js';

export class Validator {
  constructor(schema, options = {}) {
    this.schema = schema;
    this.useWorkers = options.useWorkers || false;
    this.workerThreshold = options.workerThreshold || 50; // Use workers for batches > 50 items
  ...
  async validateBatch(items) {
    // Use worker threads for large batches if enabled
    if (this.useWorkers && items.length >= this.workerThreshold) {
      return this._validateBatchWithWorkers(items);
    }

    return this._validateBatchSync(items);
  }

  _validateBatchSync(items) {
    const results = {
      valid: [],
      invalid: [],
      summary: {
  ...
  async _validateBatchWithWorkers(items) {
    const pool = getValidatorPool();
    await pool.init();

    try {
      const validationResults = await pool.validateBatch(this.schema, items);
      ...
      validationResults.forEach(result => {
        ...
        if (result.valid) {
          results.valid.push({ index: result.index, item });
          results.summary.valid++;
        } else {
          results.invalid.push({
            index: result.index,
            item,
            errors: this._formatErrors(result.errors)
          });
          results.summary.invalid++;
        }
      });
      ...
      // Fallback to sync validation if workers fail
      console.warn('Worker validation failed, falling back to sync:', error.message);
      return this._validateBatchSync(items);
    }
  }

  async validateSingle(item) {
    if (this.useWorkers) {
      const pool = getValidatorPool();
      await pool.init();

      try {
        const result = await pool.validate(this.schema, item);
        return {
          valid: result.valid,
          errors: this._formatErrors(result.errors)
        };

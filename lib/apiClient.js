...
export class OpenRouterClient {
  constructor(config = {}) {
    ...
    // Don't throw here, let the methods handle missing API key
    this._initClient();
  }
    ...
    for (let attempt = 0; attempt < this.retryAttempts; attempt++) {
      try {
        ...
        if (response.data?.choices?.[0]?.message?.content) {
          ...
          // For text format, return raw content
          if (options.format === 'text') {
            if (options.verbose) {
              return {
                data: content,
          ...
          // Return both parsed content and metadata if verbose mode is enabled
          if (options.verbose) {
            return {
              data: parsedContent,
              usage: response.data.usage,
  ...
  async getModels() {
    ...
    try {
      ...
      return models.sort((a, b) => {
        // If both have created dates, sort by newest first
        if (a.created && b.created) {
          return new Date(b.created) - new Date(a.created);
        }
        // If only one has created date, put it first
  ...
  async getUserInfo() {
    this._ensureApiKey();
    try {
      // Try to get user info from OpenRouter API
      // Different endpoints might be available
      const endpoints = [
        '/auth/key', // Check API key info
        '/user/credits', // User credits
        '/user/info', // User info
        '/me' // Common user endpoint
      ];
      ...
      // If no endpoint works, return null
      return {
        success: false,
